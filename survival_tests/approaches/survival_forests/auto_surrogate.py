...
import numpy as np
import pandas as pd
from ax import ChoiceParameter, Models, ParameterType, RangeParameter, SearchSpace, SimpleExperiment, modelbridge, optimize
...
class SurrogateAutoSurvivalForest:
    ...
    def resolve_risk_function(self, function, alpha_poly, alpha_exp, exp_threshold):
        if function == "polynomial":
            ...
            def risk_function(x):
                return np.minimum((-1) * alpha_exp * np.log(1.0 - x), exp_threshold)
    ...
    def evaluate_parameterization(self, parameterization, weight=None):
        risk_func = self.resolve_risk_function(parameterization["risk_function"], parameterization["alpha_poly"], parameterization["alpha_exp"], parameterization["exp_threshold"])
        evaluation = {"par10": self.evaluate_surrogate(risk_func, self.val_event_times, self.val_survival_functions, self.Y_val)}
    ...
    def fit(self, scenario: ASlibScenario, fold: int, num_instances: int):
        ...
        self.val_event_times, self.val_survival_functions = self.predict_survival_functions(X_val, val_imputer, val_scaler, val_models)
        ...
        for gpei_trials in gpei_list:
            try:
                search_space = SearchSpace(
                    parameters=[
                        ChoiceParameter(name="risk_function", values=["polynomial", "exponential"], parameter_type=ParameterType.STRING),
                        RangeParameter(name="alpha_poly", lower=1.0, upper=5.0, parameter_type=ParameterType.FLOAT),
                        RangeParameter(name="alpha_exp", lower=0.0, upper=1.0, parameter_type=ParameterType.FLOAT),
                        RangeParameter(name="exp_threshold", lower=1.0, upper=10.0, parameter_type=ParameterType.FLOAT),
                    ]
                )

                experiment = SimpleExperiment(
                    name="risk_function_parametrisation", search_space=search_space, evaluation_function=self.evaluate_parameterization, objective_name="par10", minimize=True
                )
    ...
    def get_x_y(self, scenario: ASlibScenario, num_requested_instances: int, algorithm_id: int, fold: int):
        amount_of_training_instances = min(num_requested_instances, len(scenario.instances)) if num_requested_instances > 0 else len(scenario.instances)
        resampled_scenario_feature_data, resampled_scenario_performances = resample(scenario.feature_data, scenario.performance_data, n_samples=amount_of_training_instances, random_state=fold)

        X_for_algorithm_id, y_for_algorithm_id = self.construct_dataset_for_algorithm_id(
            resampled_scenario_feature_data, resampled_scenario_performances, algorithm_id, scenario.algorithm_cutoff_time
        )

        return X_for_algorithm_id, y_for_algorithm_id

    def construct_dataset_for_algorithm_id(self, instance_features, performances, algorithm_id: int, algorithm_cutoff_time):
        ...
        status_and_performance_of_algorithm_with_id = np.empty(dtype=[("cens", np.bool), ("time", np.float)], shape=instance_features.shape[0])
    ...
    def fit_thresh_regressor(self, X_train, y_train, random_state):
        ...
        thresh_regressor = RandomForestRegressor(
    ...
    def fit_regressors(self, features, performances, random_state):
        ...
        models = [
            ...
            for _ in range(self.num_algorithms)
        ]
