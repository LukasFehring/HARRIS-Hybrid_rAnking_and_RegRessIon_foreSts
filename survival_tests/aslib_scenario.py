...
class ASlibScenario(object):
    ...
    def read_from_csv(self, perf_fn: str, feat_fn: str, objective: str, runtime_cutoff: float, maximize: bool, cv_fn: str = None):
        """
        ...
        self.runstatus_data = pd.DataFrame(data=np.array([["ok"] * len(self.algorithms)] * len(self.instances)), index=self.performance_data.index, columns=self.performance_data.columns)

        if objective == "runtime":
            self.runstatus_data[self.performance_data >= runtime_cutoff] = "timeout"

        self.feature_runstatus_data = pd.DataFrame(data=["ok"] * len(self.instances), index=self.instances, columns=["all"])
    ...
    def read_description(self, fn):
        ...
        self.performance_measure = description.get("performance_measures") if isinstance(description.get("performance_measures"), list) else [description.get("performance_measures")]
        ...
        self.performance_type = performance_type if isinstance(performance_type, list) else [performance_type]
        ...
        if not self.features_cutoff_time or self.features_cutoff_time == "?":
            ...
            self.logger.debug("Assumption FEATURES_CUTOFF_TIME == ALGORITHM_CUTOFF_TIME ")
        ...
        if feature_intersec:
            self.logger.warning("Intersection of deterministic and stochastic features is not empty: %s" % (str(feature_intersec)))
        algo_intersec = set(self.algortihms_deterministics).intersection(self.algorithms_stochastic)
        if algo_intersec:
            self.logger.warning("Intersection of deterministic and stochastic algorithms is not empty: %s" % (str(algo_intersec)))
    ...
    def read_algorithm_runs(self, fn):
        ...
        with open(fn, "r") as fp:
            try:
                ...
                self.logger.error("Parsing of arff file failed (%s) - maybe conflict of header and data." % (fn))
        ...
        for performance_measure in self.performance_measure:
            if arff_dict["attributes"][3 + i][0].upper() != performance_measure.upper():
                self.logger.error('"%s" as attribute is missing in %s' % (performance_measure, fn))
                sys.exit(3)
            i += 1
        ...
        perf_data = pd.DataFrame(arff_dict["data"], columns=algo_inst_col + perf_col + status_col)

        # group performance data by mean value across repetitions
        for perf in self.performance_measure:
            self.performance_data_all.append(perf_data.groupby(["instance_id", "algorithm"]).median().unstack("algorithm")[perf])

        self.performance_data = self.performance_data_all[0]

        # group runstatus by most frequent runstatus across repetitions
        self.runstatus_data = perf_data.groupby(["instance_id", "algorithm"])["runstatus"].aggregate(lambda x: collections.Counter(x).most_common(1)[0][0]).unstack("algorithm")
    ...
    def read_feature_values(self, fn):
        ...
        for f_name in arff_dict["attributes"][2:]:
            ...
            if not f_name in feature_set:
                self.logger.error('Feature "%s" was not defined as deterministic or stochastic' % (f_name))
                sys.exit(3)
        ...
        for data in arff_dict["data"]:
            ...
            if len(features) != len(self.features):
                self.logger.error("Number of features in attributes does not match number of found features; instance: %s" % (inst_name))
                sys.exit(3)
            ...
            if (inst_name, repetition) in pairs_inst_rep:
                self.logger.warning("Pair (%s,%s) is not unique in %s" % (inst_name, repetition, fn))
            else:
    ...
    def read_feature_costs(self, fn):
        ...
        if arff_dict["attributes"][1][0].upper() != "REPETITION":
            self.logger.error('"repetition" as second attribute is missing in %s' % (fn))
            sys.exit(3)
        found_groups = list(map(str, sorted(map(lambda x: x[0], arff_dict["attributes"][2:]))))
        for meta_group in self.feature_group_dict.keys():
            if meta_group not in found_groups:
                self.logger.error('"%s" as attribute is missing in %s' % (meta_group, fn))
                sys.exit(3)
        ...
        imputed_feature_cost_data = pd.DataFrame(data[:, 1:], columns=cols, dtype=np.float)
        ...
        self.feature_cost_data = self.feature_cost_data.groupby(["instance_id"]).median()
    ...
    def read_feature_runstatus(self, fn):
        ...
        for f_name in arff_dict["attributes"][2:]:
            ...
            if not f_name in self.feature_group_dict.keys():
                self.logger.error('Feature step "%s" was not defined in feature steps' % (f_name))
                sys.exit(3)

        if len(self.feature_group_dict.keys()) != len(arff_dict["attributes"][2:]):
            self.logger.error(
                "Number of feature steps in description.txt (%d) and feature_runstatus.arff (%d) does not match." % (len(self.feature_group_dict.keys()), len(arff_dict["attributes"][2:-1]))
            )
            sys.exit(3)
        ...
        self.feature_runstatus_data = self.feature_runstatus_data.groupby(["instance_id"]).aggregate(lambda x: collections.Counter(x).most_common(1)[0][0])
    ...
    def read_ground_truth(self, fn):
        ...
        data = np.array(arff_dict["data"])
        cols = list(map(lambda x: x[0], arff_dict["attributes"][1:]))
        self.ground_truth_data = pd.DataFrame(data=data[:, 1:], index=data[:, 0].tolist(), columns=cols)
    ...
    def read_cv(self, fn):
        ...
        data = np.array(arff_dict["data"])
        cols = list(map(lambda x: x[0], arff_dict["attributes"][1:]))
        self.cv_data = pd.DataFrame(data[:, 1:], index=data[:, 0], columns=cols, dtype=np.float)
    ...
    def check_data(self):
        ...
        for perf_type_i, perf_type in enumerate(self.performance_type):
            ...
            if perf_type == "runtime":
                ...
                self.performance_data_all[perf_type_i][self.runstatus_data != "ok"] = self.algorithm_cutoff_time * 10

            if perf_type == "solution_quality" and self.maximize[perf_type_i]:
                self.logger.info("Multiply all performance data by -1, since autofolio minimizes the scores but the objective is to maximize")
                self.performance_data_all[perf_type_i] *= -1

        all_data = [self.feature_data, self.feature_cost_data, self.feature_runstatus_data, self.ground_truth_data, self.cv_data]
    ...
    def get_split(self, indx=1):
        ...
        if self.cv_data is None:
            self.logger.warning("The ASlib scenario has not provided any cv.arff; create CV split...")
            self.create_cv_splits()
    ...
    def create_cv_splits(self, n_folds: int = 10):
        ...
        self.cv_data = pd.DataFrame(data=np.zeros(len(self.instances)), index=self.instances, columns=["fold"], dtype=np.float)
