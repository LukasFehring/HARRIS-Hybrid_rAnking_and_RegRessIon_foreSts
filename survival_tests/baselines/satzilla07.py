...
class SATzilla07:
    ...
    def fit(self, scenario: ASlibScenario, fold: int, num_instances: int):
        ...
        sfs_params = {"estimator": base_model, "k_features": "best", "forward": True, "scoring": scorer, "cv": 2}

        for num in range(self._num_algorithms):
            feature_selector = SequentialFeatureSelector(**sfs_params)
            feature_selector = feature_selector.fit(features, performances[:, num])
            self._features[num] = feature_selector.k_feature_idx_

            features_tmp = PolynomialFeatures(2).fit_transform(features[:, self._features[num]])

            feature_selector = SequentialFeatureSelector(**sfs_params)
            feature_selector = feature_selector.fit(features_tmp, performances[:, num])
    ...
    def predict(self, features):
        ...
        for num in range(self._num_algorithms):
            ...
            features_tmp = PolynomialFeatures(2).fit_transform(features[:, self._features[num]])
            predictions[num] = self._model[num].predict(features_tmp[self._quad_features[num]])
