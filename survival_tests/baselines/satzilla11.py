...
class SATzilla11:
    ...
    def predict(self, features, instance_id: int):
        ...
        most_voted = set(alg for alg, votes in counter.items() if votes >= max_votes)
        ...
        if len(most_voted) > 1:
            indices = [(i, j) for (i, j) in self._pairwise_indices if i in most_voted and j in most_voted]
